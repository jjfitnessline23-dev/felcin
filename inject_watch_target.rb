...
# Copy icon PNGs generated by the codemagic "Generate Watch app icons" step
ICON_SRC = "#{WATCH_SRC}/Assets.xcassets/AppIcon.appiconset"
ICON_SIZES = %w[80 88 92 100 102 108 1024]
ICON_SIZES.each do |size|
  fname = "AppIcon-#{size}.png"
  src = "#{ICON_SRC}/#{fname}"
  FileUtils.cp(src, DEST_DIR) if File.exist?(src)
end
...
# Add icon PNGs as explicit resource files
icon_refs = ICON_SIZES.map do |size|
  fname = "AppIcon-#{size}.png"
  ref = watch_group.new_file(fname)
  ref.source_tree = '<group>'
  ref.last_known_file_type = 'image.png'
  ref
end
...
# Add icon PNGs to Watch target resources
icon_refs.each { |ref| watch_target.resources_build_phase.add_file_reference(ref) }
